		    ...
		    hash_fun_t hashfun)
{
	...
	if (hash) { /* 3 */
		...
		if (hash->table) { /* 5 */
			...
			hash->function = hashfun ? hashfun : pearson16;
...
void hash_insert(hash_t *hash, hnode_t *node, const void *key, size_t len)
{
	...
	assert (hash_lookup(hash, key, len) == NULL); /* 2 */

	if (hash->dynamic && hash->nodecount >= hash->highmark) /* 3 */
		grow_table(hash);

	hkey = hash->function(key, len);
...
hnode_t *hash_lookup(const hash_t *hash, const void *key, size_t len)
{
	hash_val_t hkey, chain;
	hnode_t *nptr;

	hkey = hash->function(key, len); /* 1 */
 ...
 * and traverse.
 * Notes:
 * 1. The node must belong to this hash table (this is not asserted), and its
 * key must not have been tampered with.
...
static int hash_comp_default(const void *key1, const void *key2)
{
	return strncmp(key1, key2, HASH_VAL_T_MAX);
}
