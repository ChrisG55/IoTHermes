...
/*
 * Copied from table 1 in "Fast Hashing of Variable-length Text String" by
 * Peter K. Pearson.
 */
...
hash_val_t pearson16(const void *buf, size_t len)
{
	...
	for (i = 0; i < len; i++)
		H1 = lookupTable[H1 ^ C[i]];

	H2 = lookupTable[H2 ^ C[0] + 1 & UCHAR_MAX];
	for (i = 1; i < len; i++)
		H2 = lookupTable[H2 ^ C[i]];
